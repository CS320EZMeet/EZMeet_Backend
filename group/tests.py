...
class groupView(SimpleTestCase):
    ...
    def test_groupjoin_user_not_in_body(self):
        data={"notIt": "notIt"}
        response = self.client.post('/group/invite/1/', data=json.dumps(data), content_type="application/json")
    ...
    def test_groupleave_invalid_user(self):
        ...
        response = self.client.post('/group/leave/1/', data=json.dumps(data), content_type="application/json")
        self.assertEqual(response.status_code, 403)
        content = json.loads(response.content)
        self.assertEqual(content['message'], 'User not found')
    
    def test_groupleave_user_not_in_group(self):
